...
from lsmgridtrack import config, image, registration, kinematics2d
...
def main(
    ...
    results = kinematics2d.get_kinematics(options.grid, options.image, transform)
    kinematics2d.write_kinematics_to_vtk(results, vtk_out)
    if excel_out:
        kinematics2d.write_kinematics_to_excel(results, excel_out)
