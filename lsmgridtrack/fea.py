...
class model(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.options = FixedDict(
            {
                "Image": FixedDict({"filename": False}),
                "Region": FixedDict(
                    {
                        "origin": False,
                        "x_length": False,
    ...
    def _createMesh(self):
        e = 1e-14
        x_domain = np.arange(
            self.options["Region"]["origin"][0],
            self.options["Region"]["origin"][0]
            + self.options["Region"]["x_length"]
            + e,
            self.options["Model"]["x_edge"],
        )
        y_domain = np.arange(
            self.options["Region"]["origin"][1],
            self.options["Region"]["origin"][1]
            + self.options["Region"]["y_length"]
            + e,
            self.options["Model"]["y_edge"],
        )
        z_domain = np.arange(
            self.options["Region"]["origin"][2],
            self.options["Region"]["origin"][2]
            + self.options["Region"]["z_length"]
            + e,
    ...
    def _mapProperties(self):
        ...
        axes = numpy_support.vtk_to_numpy(
            result.GetCellData().GetArray("Orientation Distribution Function")
        )
        shape = [i - 1 for i in result.GetDimensions()][::-1] + [3, 3]
        axes = axes.reshape(shape)
        lengths = np.linalg.norm(axes, axis=4)
        scale = (3.0 / (4.0 * np.pi * np.product(lengths, axis=3))) ** (1.0 / 3.0)
        axes *= scale[:, :, :, np.newaxis, np.newaxis]

        arr = numpy_support.numpy_to_vtk(
            axes.ravel(), deep=True, array_type=vtk.VTK_FLOAT
        )
        ...
        intensity = numpy_support.vtk_to_numpy(
            result.GetCellData().GetArray("Intensity")
        )

        yg = self.options["Model"]["ground_stiffness"][0]
        mg = (
            self.options["Model"]["ground_stiffness"][1]
            - self.options["Model"]["ground_stiffness"][0]
        )
        ground_stiffness = yg + mg * intensity
        ground_stiffness = numpy_support.numpy_to_vtk(
            ground_stiffness.ravel(), deep=True, array_type=vtk.VTK_FLOAT
        )
        ...
        mf = (
            self.options["Model"]["fibre_stiffness"][1]
            - self.options["Model"]["fibre_stiffness"][0]
        )
        fibre_stiffness = yf + mf * intensity
        fibre_stiffness = numpy_support.numpy_to_vtk(
            fibre_stiffness.ravel(), deep=True, array_type=vtk.VTK_FLOAT
        )
    ...
    def writeToFEBio(self, name=None):
        ...
        nodes = (
            numpy_support.vtk_to_numpy(points.GetData())
            * self.options["Model"]["conversion_factor"]
        )
        ...
        m.addElementSet(setname="cartilage", eids=list(range(1, len(m.elements) + 1)))
        self.model = febio.Model(
            modelfile="{:}.feb".format(name), steps=[{"Displace": "solid"}]
        )

        mats = []
        gm = numpy_support.vtk_to_numpy(
            self.mesh.GetCellData().GetArray("Ground Modulus")
        )
        gf = numpy_support.vtk_to_numpy(
            self.mesh.GetCellData().GetArray("Fibre Modulus")
        )
        odfs = numpy_support.vtk_to_numpy(
            self.mesh.GetCellData().GetArray("Orientation Distribution Function")
        ).reshape(-1, 3, 3)
        if self.options["Model"]["isotropic"]:
            for i in range(3):
                for j in range(3):
                    if i == j:
                        odfs[:, i, j] = (3.0 / (np.pi * 4.0)) ** (1.0 / 3.0)
        for i in range(gm.shape[0]):
            m.addElementSet(setname="e{:d}".format(i + 1), eids=[i + 1])
            mats.append(
                febio.MatDef(
                    matid=i + 1,
                    mname="e{:d}".format(i + 1),
                    mtype="solid mixture",
                    ...
                    attributes={
                        "mat_axis": [
                            "vector",
                            "{:.6E},{:.6E},{:.6E}".format(
                                odfs[i, 0, 0], odfs[i, 0, 1], odfs[i, 0, 2]
                            ),
                            "{:.6E},{:.6E},{:.6E}".format(
                                odfs[i, 1, 0], odfs[i, 1, 1], odfs[i, 1, 2]
                            ),
                        ]
                    },
                )
            ...
            mats[-1].addBlock(
                ...
                btype="solid",
                mtype="neo-Hookean",
                attributes={"E": "{:.6E}".format(gm[i]), "v": "0.15"},
            )
            mats[-1].addBlock(
                branch=1,
                btype="solid",
        ...
        for i in range(snodes.shape[0]):
            displacement = np.array(
                transform.TransformPoint(np.array(m.nodes[snodes[i]][1:]) * 1000.0)
            ) / 1000.0 - np.array(m.nodes[snodes[i]][1:])
            boundary.addPrescribed(
                step=0,
                nodeid=snodes[i] + 1,
                dof="x",
                lc="1",
                scale=str(displacement[0]),
            )
            boundary.addPrescribed(
                step=0,
                nodeid=snodes[i] + 1,
        ...
        for k, v in self.options.items():
            for k2, v2 in v.items():
                if not v2 and k2 != "isotropic":
                    raise ValueError(
                        "Option, {:s}, in section, {:s}, must be specified to generate the model.".format(
                            k2, k
                        )
                    )
        self._readImage()
